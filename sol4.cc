...
Expr eval_under_env(Expr e, std::map<string, Expr> env) {
    return std::visit(overload {
        ...
        [&](AUnit& au) { /* TODO */ return Expr(AUnit()); },
        [&](box<struct IsAUnit>& isa) { 
            /* TODO */
            Expr expr = eval_under_env(isa->e, env);
            if (is<AUnit>(expr)) return Expr(Int(1));
        ...
        [&](box<struct MLet>& l) {
            /* TODO */
            std::map<string,Expr> env2 = makeNewEnvFrom(env);
            env2.insert_or_assign(l->varName, eval_under_env(l->e1, env));
            return eval_under_env(l->e2, env2);
        },
        [&](box<struct Fun>& f) {
            /* TODO */
        ...
        [&](box<struct APair>& ap) {
            /* TODO */
            return Expr(APair(eval_under_env(ap->e1, env), eval_under_env(ap->e2, env)));
        },
        [&](box<struct Fst>& fst) { 
            /* TODO */
            Expr e = eval_under_env(fst->e, env);
            if (!is<APair>(e)) {
                throw std::runtime_error("Type error: Fst");
            }
        ...
        [&](box<struct Snd>& snd) { 
            ...
            if (!is<APair>(e)) {
                throw std::runtime_error("Type error: Snd");
            }
            APair pair = *std::get<box<struct APair>>(e);
            return Expr(pair.e2);
        },
        [&](box<struct Call>& call) {
            ...
            std::map<string, Expr> env2 = makeNewEnvFrom(closure.env);
            if (!is<Closure>(e1)) {
                throw std::runtime_error("Type error: Closure");
            }
            env2.insert_or_assign(closure.f.funName, closure);
            env2.insert_or_assign(closure.f.argName, e2);
            Expr result = eval_under_env(closure.f.body, env2);
            return result;
        },
      }, e);
}
...
Expr ToMuplList(List<Expr> list) {
    ...
    if (list.isEmpty()) {
        ...
        return Expr(APair(list.head(),ToMuplList(list.tail())));
    }
}
...
List<Expr> FromMuplList(Expr muplList) {
    ...
    if (is<AUnit>(muplList)) {
        ...
        return List<Expr>(pair.e1, FromMuplList(pair.e2));
    }
...
Expr MuplMap() {
    ...
    Expr is_aunit = IfAUnit(Var("lst"), AUnit(), APair(
        Call(Var("fun_arg"), Fst(Var("lst"))),
        Call(Var("muplrec"), Snd(Var("lst")))
    ));
    Expr muplrec = Fun("muplrec", "lst", is_aunit);
    Expr result = Fun("outer","fun_arg",muplrec);
...
Expr MuplMapAddN() {
    ...
    Expr map = MuplMap();
    Expr inner_fun = Fun("", "x", Add(Var("x"), Var("I")));
    Expr map_call = Call(map, inner_fun);
    Expr result = Fun("", "I", map_call);
